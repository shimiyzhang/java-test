package encryptAndSafety.symmetricEncryption;
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // 原文:
        String messageCBC = "Hello, world!";
        System.out.println("Message: " + messageCBC);
        // 128位密钥 = 16 bytes Key:
        byte[] keyCBC = "1234567890abcdef".getBytes(StandardCharsets.UTF_8);
        // 加密:
        byte[] dataCBC = message.getBytes(StandardCharsets.UTF_8);
        byte[] encryptedCBC = encryptCBC(keyCBC, dataCBC);
        System.out.println("Encrypted: " + Base64.getEncoder().encodeToString(encrypted));
        // 解密:
        byte[] decryptedCBC = decryptCBC(keyCBC, encryptedCBC);
        System.out.println("Decrypted: " + new String(decryptedCBC, StandardCharsets.UTF_8));
        // 在CBC模式下，需要一个随机生成的16字节IV参数，必须使用SecureRandom生成。
        // 因为多了一个IvParameterSpec实例，因此，初始化方法需要调用Cipher的一个重载方法并传入IvParameterSpec。
        //
