package encryptAndSafety.hashAlgorithm;
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        printHexString("hello".hashCode()); // 0x5e918d2
        printHexString("hello, java".hashCode()); // 0x7a9d88e8
        printHexString("hello, bob".hashCode()); // 0xa0dbae2f
        // 两个相同的字符串永远会计算出相同的hashCode，否则基于hashCode定位的HashMap就无法正常工作。
        // 这也是为什么当我们自定义一个class时，覆写equals()方法时我们必须正确覆写hashCode()方法。

        // 哈希碰撞
        // 哈希碰撞是指，两个不同的输入得到了相同的输出：
        printHexString("AaAaAa".hashCode()); // 0x7460e8c0
        printHexString("BBAaBB".hashCode()); // 0x7460e8c0
        // 有童鞋会问：碰撞能不能避免？答案是不能。
        // 碰撞是一定会出现的，因为输出的字节长度是固定的，String的hashCode()输出是4字节整数，最多只有4294967296种输出，
        ...
        // 这样一来，数据库管理员看不到用户的原始口令。
        // 即使数据库泄漏，黑客也无法拿到用户的原始口令。
        ...
        byte[] value = md.digest(); // 20 bytes: db8ac1c259eb89d4a131b253bacfca5f319d54f2
        System.out.println(new BigInteger(1, value).toString(16));
    ...
    // 输出16进制字符串
    static void printHexString(int input) {
        System.out.println("0x" + Integer.toHexString(input));
    }
}
