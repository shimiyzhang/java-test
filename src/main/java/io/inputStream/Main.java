...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // 这些底层错误由Java虚拟机自动封装成IOException异常并抛出。
        ...
        try (InputStream input = new ByteArrayInputStream(data1)) {
            String ss = readAsString(input);
            System.out.println(ss);
        }

        // 这就是面向抽象编程原则的应用：
        // 接受InputStream抽象类型，而不是具体的FileInputStream类型，
        // 从而使得代码可以处理InputStream的任意实现类。
    }

    // 下面的代码演示了如何完整地读取一个FileInputStream的所有字节：
    public static void readFile() throws IOException {
        // 创建一个FileInputStream对象:
        // InputStream input = new FileInputStream("D:\\codeTime\\java-test\\README.md");
        // for (;;) {
        ...
        try {
            input = new FileInputStream("D:\\codeTime\\java-test\\README.md");
            int n;
        ...
        // 推荐的写法如下：
        try (InputStream inputStream = new FileInputStream("D:\\codeTime\\java-test\\README.md")) {
            int n;
    ...
    public static void readFileByBytes() throws IOException {
        try (InputStream input = new FileInputStream("D:\\codeTime\\java-test\\README.md")) {
            // 定义1000个字节大小的缓冲区：
