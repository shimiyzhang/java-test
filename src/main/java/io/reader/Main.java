...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // java.io.Reader是所有字符输入流的超类，它的主要方法是：
        ...
        Reader readerUTF8 = new FileReader("src/readme.txt", StandardCharsets.UTF_8);

        // 和InputStream类似，Reader也是一种资源，需要保证出错的时候也能正确关闭，
        // 所以我们需要用try (resource)来保证Reader在无论有没有IO错误的时候都能够正确地关闭：
        try (Reader readerAutoClose = new FileReader("src/readme.txt")) {
            System.out.println(readerAutoClose);
        }
        ...
        try (Reader readerBuffer = new FileReader("src/readme.txt", StandardCharsets.UTF_8)) {
            char[] buffer = new char[1000];
            int n;
            while ((n = readerBuffer.read(buffer)) != -1) {
        ...
        try (Reader readerCharArray = new CharArrayReader("Hello".toCharArray())) {
            for (; ; ) {
                int n = readerCharArray.read(); // 反复调用read()方法，直到返回-1
                if (n == -1) {
                    break;
                }
                System.out.println((char) n); // 打印char
        ...
        try (Reader readerString = new StringReader("World")) {
            for (; ; ) {
                int n = readerString.read(); // 反复调用read()方法，直到返回-1
                if (n == -1) {
                    break;
                }
                System.out.println((char) n); // 打印char
        ...
        // 变换为Reader:
        Reader readerStream = new InputStreamReader(input, StandardCharsets.UTF_8);
        // 构造InputStreamReader时，我们需要传入InputStream，还需要指定编码，就可以得到一个Reader对象。
        // 上述代码可以通过try (resource)更简洁地改写如下：
        try (Reader readerStreamAutoClose = new InputStreamReader(new FileInputStream("src/readme.txt"), StandardCharsets.UTF_8)) {
            for (; ; ) {
                int n = readerStreamAutoClose.read(); // 反复调用read()方法，直到返回-1
                if (n == -1) {
                    break;
                }
                System.out.println((char) n); // 打印char
