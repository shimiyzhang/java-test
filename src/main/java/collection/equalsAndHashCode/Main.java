package collection.equalsAndHashCode;
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(map.get("a")); // Person("Xiao Ming")
        ...
        System.out.println(map1.get(key2)); // 123

        System.out.println(key1 == key2); // false
        System.out.println(key1.equals(key2)); // true
        // 因为在Map的内部，对key做比较是通过equals()实现的，这一点和List查找元素需要正确覆写equals()是一样的，
        ...
        Map<String, String> hasSizeMap = new HashMap<>(10000);
        // 虽然指定容量是10000，但HashMap内部的数组长度总是2^n，因此，实际数组长度被初始化为比10000大的16384（214）。

        // 最后一个问题：如果不同的两个key，例如"a"和"b"，它们的hashCode()恰好是相同的
        // （这种情况是完全可能的，因为不相等的两个实例，只要求hashCode()尽量不相等），
        // 那么，当我们放入：
        // map.put("a", new Person("Xiao Ming"));
        // map.put("b", new Person("Xiao Hong"));
        // 时，由于计算出的数组索引相同，后面放入的"Xiao Hong"会不会把"Xiao Ming"覆盖了？
        //
        // 当然不会！使用Map的时候，只要key不相同，它们映射的value就互不干扰。
        // 但是，在HashMap内部，确实可能存在不同的key，映射到相同的hashCode()，即相同的数组索引上，肿么办？

        // 我们就假设"a"和"b"这两个key最终计算出的索引都是5，
        ...
        // 在查找的时候，例如：
        // Person p = map.get("a");
        // HashMap内部通过"a"找到的实际上是List<Entry<String, Person>>，
        // 它还需要遍历这个List，并找到一个Entry，它的key字段是"a"，才能返回对应的Person实例。

        // 我们把不同的key具有相同的hashCode()的情况称之为哈希冲突。
        // 在冲突的时候，一种最简单的解决办法是用List存储hashCode()相同的key-value。
        // 显然，如果冲突的概率越大，这个List就越长，Map的get()方法效率就越低，这就是为什么要尽量满足条件二：
        //
        //  如果两个对象不相等，则两个对象的hashCode()尽量不要相等。
        //
...
class Person {
    ...
    @Override
    public int hashCode() {
        // 注意到String类已经正确实现了hashCode()方法，
        // 我们在计算Person的hashCode()时，反复使用31*h，
        // 这样做的目的是为了尽量把不同的Person实例的hashCode()均匀分布到整个int范围。
        // int result = 17;
